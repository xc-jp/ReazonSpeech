...
def audio_to_file(fp: IO, audio: AudioData, format: str | None = "wav"):
...
def norm_audio(audio: AudioData) -> AudioData:
    ...
    if audio.samplerate != SAMPLERATE:
        waveform = librosa.resample(
            waveform, orig_sr=audio.samplerate, target_sr=SAMPLERATE
        )
...
def pad_audio(audio: AudioData, seconds: float) -> AudioData:
    ...
    """
    waveform = np.pad(
        audio.waveform, pad_width=int(seconds * audio.samplerate), mode="constant"
    )
